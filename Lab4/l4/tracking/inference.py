...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variables.append(PAC) #adding variables to the variables list
    variables.append(GHOST0)
    variables.append(GHOST1)
    variables.append(OBS0)
    variables.append(OBS1)
    print(variables)
                           #adding edges to the edges list
    tup0 = (GHOST0, OBS0)
    tup1 = (PAC, OBS0)
    tup2 = (PAC, OBS1)
    tup3 = (GHOST1, OBS1)
    ...
    variableDomainsDict[PAC] = [] #creating an empty list for the domain of each variable
    variableDomainsDict[GHOST0] = []
    variableDomainsDict[GHOST1] = []
    variableDomainsDict[OBS0] = []
    variableDomainsDict[OBS1] = []

    for x in range(X_RANGE):              #adding the domain for each variable
        for y in range(Y_RANGE):          #the domain of ghosts and pacman are same 
            variableDomainsDict[PAC].append((x,y))
            variableDomainsDict[GHOST0].append((x,y))
            variableDomainsDict[GHOST1].append((x,y))
#the domain for the observer0 and observer1 is all possible  manhattan distances  between pac and the ghosts
    for pos1 in variableDomainsDict[PAC]:
        for pos2 in variableDomainsDict[GHOST0]:
            if abs(manhattanDistance(pos1,pos2)+MAX_NOISE) not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS0].append(abs(manhattanDistance(pos1,pos2)+MAX_NOISE))
            if abs(manhattanDistance(pos1,pos2)-MAX_NOISE) not in variableDomainsDict[OBS0]:
                variableDomainsDict[OBS0].append(abs(manhattanDistance(pos1,pos2)-MAX_NOISE))
    variableDomainsDict[OBS1]=variableDomainsDict[OBS0]
    # for x in range((X_RANGE + Y_RANGE -1 + MAX_NOISE)):
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder: #take a variable from the elimination order and join all the factors that contain this variable and return a tuple containing list of not joined factors and joined factor
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, variable)
    
            if len(joinedFactor.unconditionedVariables()) > 1:# for eliminating the variable from the joined factor no of unconditioned variables should be greater than 1
                incrementallyMarginalizedJoint = eliminate(joinedFactor, variable)#eliminating the variable from the joined factor
                currentFactorsList.append(incrementallyMarginalizedJoint)# add the new factor to the current factors list
        #join all the factors and normalize
        fullJoint = joinFactors(currentFactorsList)
